...
Avoid interleaving style writes with layout reads. Reading layout properties between style changes forces synchronous reflows.
...
```tsx
// Incorrect: interleaving in React
...
See [CSS Triggers](https://csstriggers.com/) for layout-forcing operations.
...
`.sort()` mutates the array in place, which causes bugs with React state and props.

**Incorrect: mutates original array**

```typescript
function UserList({ users }: { users: User[] }) {
  // Mutates the users prop!
